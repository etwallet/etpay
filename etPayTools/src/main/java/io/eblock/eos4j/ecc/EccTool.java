...
public class EccTool {
	...
	public  static BigInteger  GetQFromPk(String pubk)
	{
//		byte[] addy = Base58.decode(pubk);
//		byte[] r =new byte[32];
//		byte[] temp =
//				ByteUtils.copy(te, 1, r, 0, 32);
//
//		BigInteger Q = new BigInteger(hash, 16);
      return null;
	}
//  String te = "KXss1BezmWw7tUgvR3NcoE6ZKSJVkMBiwkCcPN11UqHyaZX351NqcrtjF9QpFzZC3rCCgp6cdDxpTc6GPr6d4dzrwCAoPo"

	public static boolean verfy(String eosPk, String signature,String hash) {

		Sha256Hash hash1 = new Sha256Hash(hash);

		byte[] dpk = Base58.decode(eosPk.substring(3));
		byte[] pub = new byte[33];
		System.arraycopy(dpk, 0, pub, 0, 33);
		ECKey ecKey = ECKey.fromPublicOnly(pub);
		String te = signature.substring("SIG_K1_".length());
		byte[] Sigbyte = Base58.decode(te);
		byte[] Rbyte = new byte[32];
		byte[] Sbyte = new byte[32];
		System.arraycopy(Sigbyte, 1, Rbyte, 0, 32);
		System.arraycopy(Sigbyte, 33, Sbyte, 0, 32);

		ECKey.ECDSASignature signature1 =new ECKey.ECDSASignature(new BigInteger(Rbyte),new BigInteger(Sbyte));
	  return 	ecKey.verify(hash1,signature1);
...
//		Base58.decode()
	...
	public static String sign(String pk, String data) {
		...
		pub_buf[0] = (byte) i;
		ByteUtils.copy(sign.getR().toByteArray(), 0, pub_buf, 1, sign.getR().toByteArray().length);
		System.out.println("length"+sign.getR().toByteArray().length);
		ByteUtils.copy(sign.getS().toByteArray(), 0, pub_buf, sign.getR().toByteArray().length + 1,
				sign.getS().toByteArray().length);
		System.out.println("sign.getS().toByteArray()"+sign.getS().toByteArray().length);
		byte[] checksum = Ripemd160.from(ByteUtils.concat(pub_buf, "K1".getBytes())).bytes();
